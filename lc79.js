...
let isFetching = false; // BỔ SUNG: Cờ khóa chống gọi chồng chéo gây treo RAM
...
async function fetchAndProcessHistory() {
    // BỔ SUNG: Chống gọi chồng chéo nếu mạng lag treo API
    if (isFetching) return;
    isFetching = true;
    
    try {
        // BỔ SUNG: AbortController ngắt kết nối cưỡng bức nếu sau 4.5s API không phản hồi (chống treo)
...
app.get("/", async () => { 
    return {
        ...
        version: "8.1 AI Vi Long Absolute Core (Anti-Crash Version)", 
        algorithms: ALL_ALGS.length,
        pattern_recognition: "ULTRA VIP (Khóa ngắc 1-1 & 2-2, Bắt Đỉnh Phân Phối Poisson, Ghi Đè God Mode, Anti-Hang)",
        endpoints: [
            "/api/taixiumd5/lc79",
...
const start = async () => {
    ...
    console.log("\n🚀 AI Tài Xỉu MD5 Pro V8.1 - ULTRA GOD MODE (ANTI-CRASH) Đã Khởi Động!");
